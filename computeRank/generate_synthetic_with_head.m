function [A,A_orig,M,W,permute_vect] = generate_synthetic_with_head(d,n,k,c,eta0,eta2,beta,normalize)
% A = M * W + N;
...
tic
for i=1:k
    alpha = ones(d,1) ./ (linspace(1,d,d).^1)';
    alpha = alpha / sum(alpha);
    
    %start_ind = offset + 1;
    %end_ind = start_ind + c*k;
    %alpha(start_ind:end_ind) = alpha(start_ind:end_ind) * 3; 
    
    start_ind = c*(i-1) + 1 + offset;
    end_ind = start_ind + c;
    scaling_factor = eta0/sum(alpha(start_ind:end_ind));
