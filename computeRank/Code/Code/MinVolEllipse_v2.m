function [A , c] = MinVolEllipse_v2(P, tolerance)
...
%     inv_X=inv(X);
%     M = diag(Q' * inv_X * Q);  % M the diagonal vector of an NxN matrix
M = diag(Q' * (X \ Q));
    ...
    u = new_u;
    if mod(iter,100)==0
    fprintf('iter inside minvolellipse is %d\n',iter);
    end
    %fprintf('iter in MinVolEllipse is %d\n',iter);
    iter=iter+1;
end
     fprintf('iter inside minvolellipse stopped at %d\n',iter);
% if iter>=200
%     A=0;
%     return;
% end
