% This script generates synthetic data as per dominant assumptions
% described in paper and computes the l1 Accuracy of our proposed method. 
clear all;
close all;
n = 1000;
d = 500;
...
alpha_arr = [0.01];  % Each column of W is sampled from dirichlet with parameter 0.01
beta_arr = 0.65:0.05:0.7;
%beta_arr = [0.1];
normalize = 2;    % This says how the noise matrix will be bounded
residual_norm_param=1;
%
%funs = {@heur_spa,@spa,@fast_hull,@ER_spa_svds_v2,@ER_xray_svds_v2,@TSVD,@UTSVD};
%funs_str = {'Heur-SPA','SPA','XRAY','ER-SPA','ER-XRAY','TSVD','UTSVD'};
funs = {@UTSVD};
funs_str = {'UTSVD'};
...
for cw_count =1:length(cw_arr)
    ...
    for alpha_count=1:length(alpha_arr)
        ...
        outpath_res_norm = sprintf('/home/jagdeep/Desktop/NMF/Experiments/ICML16/synthetic_catchwords_final/res_norm/alpha-%f-cw-%d',alpha,cw);
        mkdir(outpath_res_norm);
        
        outpath_res_norm_db = sprintf('/home/jagdeep/Dropbox/NMF/Code_ICML16/synthetic_catchwords_final/res_norm/alpha-%f-cw-%d',alpha,cw);
        ...
        for beta_count=1:length(beta_arr)
            ...
            while current_iter<=no_of_iter
                for iter_count=current_iter:no_of_iter
                    ...
                    outpath_data = sprintf('/home/jagdeep/Desktop/NMF/Experiments/ICML16/synthetic_catchwords_final/data/cabi-%d-%f-%f-%d',cw,alpha,beta,iter_count);
                    mkdir(outpath_data);
                    %
                    ...
                    for alg_idx=1:num_algs
                        ...
                        if  strcmp(funs_str{alg_idx},'UTSVD')
                            outpath = sprintf('/home/jagdeep/Desktop/NMF/Experiments/ICML16/synthetic_catchwords_final/utsvd_output1/Par-%d-%f-%f-%d',cw,alpha,beta,iter_count);
                            mkdir(outpath);
                            tusvd=tic;
                        ...
                        elseif  strcmp(funs_str{alg_idx},'TSVD')
                            outpath = sprintf('/home/jagdeep/Desktop/NMF/Experiments/ICML16/synthetic_catchwords_final/tsvd_output1/Par-%d-%f-%f-%d',cw,alpha,beta,iter_count);
                            mkdir(outpath);
                            tusvd=tic;
                            [M,~] = TSVD(A,outpath,k);
                            disp('M found');
