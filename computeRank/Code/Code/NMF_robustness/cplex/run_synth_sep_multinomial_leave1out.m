...
for noise_idx = 1:length(noise_arr)
    ...
    current_iter=1;
    loop_count=1;
    outpath_residual_norm= sprintf('/home/local/ANT/pjagdeep/NMF_Experiments/synth_sep_multin_small_leave1out/residual_norm/noise%d',noise_arr);
    mkdir(outpath_residual_norm);
    
    outpath_res_norm_db= sprintf('/home/local/ANT/pjagdeep/Dropbox/NMF/synth_sep_multin_small_leave1out/residual_norm/noise%d',noise_arr);
    ...
    while current_iter<=no_of_iter && loop_count<20
        ...
        for iter_count = current_iter:no_of_iter
            ...
            [A,A_orig,B_orig,C_orig,true_index,permute_vect] = synthetic_sep_multinomial(d,n,k,noise_level);
            ...
            outpath_data = sprintf('/home/local/ANT/pjagdeep/NMF_Experiments/synth_sep_multin_small_leave1out/data/noise_iter%d_%d',noise_level,iter_count);
            mkdir(outpath_data);
            ...
            for cv_count = 1:length(rd_p)
                ...
                for alg_idx = 1:num_algs
                    if  strcmp(funs_str{alg_idx},'UTSVD')
                        outpath_utsvd = sprintf('/home/local/ANT/pjagdeep/NMF_Experiments/synth_sep_multin_small_leave1out/tsvd_out/noise_iter%d_%d',noise_level,iter_count);
                        mkdir(outpath_utsvd);
                        tusvd=tic;
                    ...
                    elseif  strcmp(funs_str{alg_idx},'TSVD')
                        outpath_tsvd = sprintf('/home/local/ANT/pjagdeep/NMF_Experiments/synth_sep_multin_small_leave1out/tsvd_out/noise_iter%d_%d',noise_level,iter_count);
                        mkdir(outpath_tsvd);
                        tusvd=tic;
