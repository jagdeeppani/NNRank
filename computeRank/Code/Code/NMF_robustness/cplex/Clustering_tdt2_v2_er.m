% function Clustering_tdt2_v2_er
clear all;
close all;
...
for k_count=1:length(k_arr)
    ...
    for repeat_count=1:no_of_repetitions
        ...
        clear fea;
        [m,n] = size(A);
        %         outpath_data = '/media/BM/Jagdeep/NMF/cplex/output_clustering/tdt_v2/data';
        outpath_data = sprintf('/media/BM/Jagdeep/NMF/cplex/output_clustering/tdt_v2/k_%d/rpt_%d/data',k,repeat_count);
        mkdir(outpath_data);
        ...
        outpath_result = sprintf('/media/BM/Jagdeep/NMF/cplex/output_clustering/tdt_v2/k_%d/rpt_%d/result',k,repeat_count);
        mkdir(outpath_result);
        
        for alg_idx = 1:num_algs
            if strcmp(funs_str{alg_idx},'UTSVD')
                disp('UTSVD started');
                outpath_tsvd = sprintf('/media/BM/Jagdeep/NMF/cplex/output_clustering/tdt_v2/k_%d/rpt_%d/tsvd',k,repeat_count);
                mkdir(outpath_tsvd);
                utime=tic;
        ...
        %
        % outpath_data = '/media/BM/Jagdeep/NMF/cplex/output_clustering/tdt2_erspa/output';
        % mkdir(outpath11);
    ...
    outpath_final_result = sprintf('/media/BM/Jagdeep/NMF/cplex/output_clustering/tdt_v2/k_%d',k);
