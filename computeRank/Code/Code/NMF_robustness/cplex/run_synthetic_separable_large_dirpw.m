...
for noise_idx = 1:length(noise_arr)
    ...
    current_iter=1;
    loop_count=1;
    outpath_residual_norm= sprintf('/home/jagdeep/Desktop/NMF/Experiments/tsvd-code-birch-v3/tsvd_cplex/Results_temp2/synthetic_separable_large/Dir_pw/residual_norm/noise%d',noise_level);
    ...
    while current_iter<=no_of_iter && loop_count<20
        ...
        for iter_count = current_iter:no_of_iter
            ...
            outpath_data = sprintf('/home/jagdeep/Desktop/NMF/Experiments/tsvd-code-birch-v3/tsvd_cplex/Results_temp2/synthetic_separable_large/Dir_pw/data/noise_iter%d_%d',noise_level,iter_count);
            mkdir(outpath_data);
            ...
            for alg_idx = 1:num_algs
                if  strcmp(funs_str{alg_idx},'UTSVD')
                    outpath_tsvd = sprintf('/home/jagdeep/Desktop/NMF/Experiments/tsvd-code-birch-v3/tsvd_cplex/Results_temp2/synthetic_separable_large/Dir_pw/tsvd_out/noise_iter%d_%d',noise_level,iter_count);
                    mkdir(outpath_tsvd);
                    tusvd=tic;
