...
alpha = 0.01;  % Each column of W is sampled from dirichlet with parameter 0.01
...
for noise_idx = 1:length(noise_arr)
    ...
    normalize = 2; % Different normalization of the Noise.
    density = 1;   % Proportion of nonzero entries in the noise matrix (but at least one nonzero per column).
    
    current_iter=1;
    loop_count=1;
    outpath_residual_norm= sprintf('/home/local/ANT/pjagdeep/NMF_Experiments/synth_dominant_gaussian_small_leave1out/residual_norm/noise%d',noise_level);
    mkdir(outpath_residual_norm);
    
    outpath_res_norm_db= sprintf('/home/local/ANT/pjagdeep/Dropbox/NMF/synth_dominant_gaussian_small_leave1out/residual_norm/noise%d',noise_level);
    ...
    while current_iter<=no_of_iter && loop_count<20
        ...
        for iter_count = current_iter:no_of_iter
            ...
            [A,A_orig,B_orig,C_orig,permute_vect] = generate_synthetic(d,n,k,cw,etta,alpha,noise_level,normalize);  % We set A = (M_orig * W_orig + N)(perm_vect,:)
            ...
            outpath_data = sprintf('/home/local/ANT/pjagdeep/NMF_Experiments/synth_dominant_gaussian_small_leave1out/data/noise_iter%d_%d',noise_level,iter_count);
            mkdir(outpath_data);
            ...
            for cv_count = 1:length(rd_p)
                ...
                for alg_idx = 1:num_algs
                    if  strcmp(funs_str{alg_idx},'UTSVD')
                        outpath_utsvd = sprintf('/home/local/ANT/pjagdeep/NMF_Experiments/synth_dominant_gaussian_small_leave1out/tsvd_out/noise_iter%d_%d',noise_level,iter_count);
                        mkdir(outpath_utsvd);
                        tusvd=tic;
                    ...
                    elseif  strcmp(funs_str{alg_idx},'TSVD')
                        outpath_tsvd = sprintf('/home/local/ANT/pjagdeep/NMF_Experiments/synth_dominant_gaussian_small_leave1out/tsvd_out/noise_iter%d_%d',noise_level,iter_count);
                        mkdir(outpath_tsvd);
                        tusvd=tic;
