...
for cw_count =1:length(cw_arr)
    ...
    for alpha_count=1:length(alpha_arr)
        ...
        outpath_res_norm = sprintf('/home/local/ANT/pjagdeep/NMF_Experiments/synth_sep_multinomial_small/res_norm/alpha-%f-cw-%d',alpha,cw);
        mkdir(outpath_res_norm);
        
        outpath_res_norm_db = sprintf('/home/local/ANT/pjagdeep/Dropbox/NMF/synth_sep_multinomial_small/res_norm/alpha-%f-cw-%d',alpha,cw);
        ...
        for beta_count=1:length(beta_arr)
            ...
            while current_iter<=no_of_iter
                for iter_count=current_iter:no_of_iter
                    ...
                    %[A,A_orig,M_orig,W_orig,true_index,permute_vect] = synthetic_sep_gaussian(d,n,k,beta);
                    [A,A_orig,M_orig,W_orig,true_index,permute_vect] = synthetic_sep_multinomial(d,n,k,beta);
                    %
                    %                     filename = strcat(outpath_data,'/A_orig.mat');
                    %             save(filename,'A_orig');
                    outpath_data = sprintf('/home/local/ANT/pjagdeep/NMF_Experiments/synth_sep_multinomial_small/data/cabi-%d-%f-%f-%d',cw,alpha,beta,iter_count);
                    mkdir(outpath_data);
                    %
                    ...
                    for alg_idx=1:num_algs
                        ...
                        if  strcmp(funs_str{alg_idx},'UTSVD')
                            outpath = sprintf('/home/local/ANT/pjagdeep/NMF_Experiments/synth_sep_multinomial_small/utsvd_output1/Par-%d-%f-%f-%d',cw,alpha,beta,iter_count);
                            mkdir(outpath);
                            tusvd=tic;
                        ...
                        elseif  strcmp(funs_str{alg_idx},'TSVD')
                            outpath = sprintf('/home/local/ANT/pjagdeep/NMF_Experiments/synth_sep_multinomial_small/tsvd_output1/Par-%d-%f-%f-%d',cw,alpha,beta,iter_count);
                            mkdir(outpath);
                            tusvd=tic;
                            [M,~] = TSVD(A,outpath,k);
                            disp('M found');
