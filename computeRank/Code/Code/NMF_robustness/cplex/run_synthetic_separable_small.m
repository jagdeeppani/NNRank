...
for noise_idx = 1:length(noise_arr)
    ...
    current_iter=1;
    loop_count=1;
    outpath_residual_norm= sprintf('/home/local/ANT/pjagdeep/NMF_Experiments/Robustness/GillisLP/residual_norm/noise%d',noise_level);
    mkdir(outpath_residual_norm);

outpath_res_norm_db= sprintf('/home/local/ANT/pjagdeep/Dropbox/NMF/Code_ICML16/NMF_robustness/GillsLP/residual_norm_05_1_2/noise%d',noise_level);
...
%             end
            ...
            outpath_data = sprintf('/home/local/ANT/pjagdeep/NMF_Experiments/Robustness/GillisLP/data/noise_iter%d_%d',noise_level,iter_count);
            mkdir(outpath_data);
            ...
            for alg_idx = 1:num_algs
                if  strcmp(funs_str{alg_idx},'UTSVD')
                    outpath_tsvd = sprintf('/home/local/ANT/pjagdeep/NMF_Experiments/Robustness/GillisLP/tsvd_out/noise_iter%d_%d',noise_level,iter_count);
                    mkdir(outpath_tsvd);
                    tusvd=tic;
