...
for beta_idx = 1:length(beta_arr)
    ...
    while current_iter<=no_of_iter && loop_count<20
        ...
        for iter_count = current_iter:no_of_iter
            outpath_data = sprintf('/media/BM/Jagdeep/NMF/cplex/semi_synthetic_spa/data/beta-%f-iter-%d',beta,iter_count);
            mkdir(outpath_data);
            ...
            %save(filename,'A');
            inpath_data = sprintf('/media/BM/Jagdeep/NMF/Results_new/semi_synthetic/data/beta-%f-iter-%d',beta,iter_count);
            current_path = pwd;
                    cd(inpath_data);
                    load matrix_A.mat;
            ...
            for alg_idx = 1:num_algs
                if  strcmp(funs_str{alg_idx},'UTSVD')
                    outpath = sprintf('/media/BM/Jagdeep/NMF/cplex/semi_synthetic_spa/tsvd_output/beta_iter%d_%d',beta,iter_count);
                    mkdir(outpath);
                    tusvd=tic;
    ...
    %     mkdir(outpath11);
    outpath_res_norm = sprintf('/media/BM/Jagdeep/NMF/cplex/semi_synthetic_spa/residual_norm/beta_%d',beta);
