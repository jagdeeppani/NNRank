% Post-processed Preconditioned Successive Projection Algorithm (Post-Prec-SPA)
% 
% *** Description ***
...
% algorithm (SPA) is run on the preconditioned matrix. Finally, the
% post-processing of Arora et al. (A Practical Algorithm for Topic Modeling
% with Provable Guarantees, ICML '13) is used to refine the solution. 
%
% See also N. Gillis and S.A. Vavasis, `Semidefinite Programming Based 
% Preconditioning for More Robust Near-Separable Nonnegative Matrix
% Factorization', arXiv, 2013. 
% 
% [K,Q,u] = PostPrecSPA(M,r,normalize,dimred,affi)
%
% ****** Input ******
...
function [K,Q,u] = PostPrecSPA(M,r,normalize,init,affi)
disp('PostPrecSPA started');
[m,n] = size(M); 
if r > min(m,n)
...
if nargin <= 3, 
    ...
    if m*n*r*min(m,n) >= 1e12, init = 3; end
end
...
% Post-processed SPA
K = FastSepNMFpostpro(Q*Mr,r); 
disp('PostPrecSPA done');