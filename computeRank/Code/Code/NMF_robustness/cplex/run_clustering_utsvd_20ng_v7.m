...
outpath11 = '/home/jagdeep/Desktop/NMF/Experiments/tsvd-code-birch-v2/tsvd_cplex/output_clustering/TwentyNG_tsvd_birch';
mkdir(outpath11);
filename0 = strcat(outpath11,'/anchor_indices_all_20ng.mat');
save(filename0,'anchor_indices_all');
%load anchor_indices_all_20ng.mat;


% Running TSVD
 %outpath = sprintf('/home/jagdeep/Desktop/NMF/Experiments/tsvd-code-birch-v2/tsvd_cplex/output/clustering/alt_min_iter');
     outpath = '/home/jagdeep/Desktop/NMF/Experiments/tsvd-code-birch-v2/tsvd_cplex/output_clustering/TwentyNG_tsvd_birch';
    mkdir(outpath);

    tic
    [M_utsvd,cluster_id2] = TSVD(A,outpath,k);
    disp('M found');
...
%         temp_Accuracy(alg_idx,alt_min_count) = sumAc/n;
 ...
 % Accuracy evaluation started
    ...
    if alt_min_count~=length(alt_min)       % No need to run on last iteration
    if  strcmp(funs_str{alg_idx},'UTSVD')
        M= nnlsHALSupdt( A' , W' );
        M=M';
        W = nnlsHALSupdt( A ,M );
