...
for noise_idx = 1:length(noise_arr)
    ...
    current_iter=1;
    loop_count=1;
    outpath_residual_norm= sprintf('/home/jagdeep/Desktop/NMF/Experiments/ICML16/Robustness/Dir_sparse_nnls/residual_norm/noise%d',noise_level);
    mkdir(outpath_residual_norm);
    
    res_norm_L1_A_temp = zeros(num_algs,m);   %To be updated
    res_norm_L2_A_temp = zeros(num_algs,m);
    res_t_norm_L1_A_temp = zeros(num_algs,m);
    res_norm_s_A_temp = zeros(num_algs,m);
    res_norm_spct_A_temp = zeros(num_algs,m);
    
    res_norm_L1_Aorig_temp = zeros(num_algs,m);
    res_norm_L2_Aorig_temp = zeros(num_algs,m);
    res_t_norm_L1_Aorig_temp = zeros(num_algs,m);
    res_norm_s_Aorig_temp = zeros(num_algs,m);
    res_norm_spct_Aorig_temp = zeros(num_algs,m);
    
    res_norm_L1_M_temp = zeros(num_algs,m);
    res_norm_L2_M_temp = zeros(num_algs,m);
    res_t_norm_L1_M_temp = zeros(num_algs,m);
    res_norm_s_M_temp = zeros(num_algs,m);
    res_norm_spct_M_temp = zeros(num_algs,m);
    
    res_norm_L1_W_temp = zeros(num_algs,m);
    res_norm_L2_W_temp = zeros(num_algs,m);
    res_t_norm_L1_W_temp = zeros(num_algs,m);
    res_norm_s_W_temp = zeros(num_algs,m);
    res_norm_spct_W_temp = zeros(num_algs,m);
    ...
    while current_iter<=no_of_iter && loop_count<20
        ...
        for iter_count = current_iter:no_of_iter
            ...
            outpath_data = sprintf('/home/jagdeep/Desktop/NMF/Experiments/ICML16/Robustness/Dir_sparse_nnls/data/noise_iter%d_%d',noise_level,iter_count);
            mkdir(outpath_data);
            ...
            for cv_count = 1:length(rd_p)
                ...
                for alg_idx = 1:num_algs
                    if  strcmp(funs_str{alg_idx},'UTSVD')
                        outpath_utsvd = sprintf('/home/jagdeep/Desktop/NMF/Experiments/ICML16/Robustness/Dir_sparse_nnls/tsvd_out/noise_iter%d_%d',noise_level,iter_count);
                        mkdir(outpath_utsvd);
                        tusvd=tic;
                        [W,~] = TSVD_new_threshold(A',outpath_utsvd,k,0);  % Birch is set inside TSVD
                        W = W';
                        disp('W found');
