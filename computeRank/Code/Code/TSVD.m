function [M_hat,cluster_id2] = TSVD(A,outpath,K,varargin)
...
% idxf= find(sum(A,1)<15);
% A(:,idxf) = [];
% fprintf('%d small docs are deleted',length(idxf));
...
%fprintf('w0 is %f\n',w0)
[B, thres] = threshold_sparse_new(A, w0, eps1, w, d);
toc;
...
% M_hat(:,l) = sum(UA * DA * (VA(alpha1,:)'),2)*1.0/n;
end
