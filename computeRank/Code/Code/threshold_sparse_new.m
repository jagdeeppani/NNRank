function [B, thres] = threshold_sparse_new(A, omega1, omega2, w, d)
...
eps = 0.001;  % WHATS A GOOD NUMBER HERE ??
...
for i =1:w
    ...
    zeta = t(index);
    while zeta ~= 0
    	if (sum(abs(t - zeta) <= eps*zeta ) < th2)
            break;
    	else 
    	  index = find(t <= (1-eps)*zeta);
