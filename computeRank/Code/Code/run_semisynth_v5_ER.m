...
funs = {@heur_spa,@spa,@ER_spa_svds};
% funs = {@spa};
funs_str = {'Heur-SPA','SPA','ER-SPA'};
% funs_str = {'SPA'};
...
for beta_idx = 1:length(beta_arr)
    ...
    while current_iter<=no_of_iter && loop_count<20
        ...
        for iter_count = current_iter:no_of_iter
            outpath_data = sprintf('/home/jagdeep/Desktop/NMF/Experiments/ICML16/semi_synthetic/data/beta-%f-iter-%d',beta,iter_count);
            mkdir(outpath_data);
            ...
            %save(filename,'A');
            %inpath_data = sprintf('/home/jagdeep/Desktop/NMF/Experiments/ICML16/semi_synthetic/data/beta-%f-iter-%d',beta,iter_count);
            %current_path = pwd;
            %        cd(inpath_data);
            %        load matrix_A.mat;
            %                           
            ...
            for alg_idx = 1:num_algs
                if  strcmp(funs_str{alg_idx},'UTSVD')
                    outpath = sprintf('/home/jagdeep/Desktop/NMF/Experiments/ICML16/tsvd_output/beta_iter%d_%d',beta,iter_count);
                    mkdir(outpath);
                    tusvd=tic;
                ...
                fprintf('%s : Ls residual norm : (%5.1f) with noise %f and iter %d\n', funs_str{alg_idx},res_norm_s_A_normalized_temp(alg_idx,iter_count), beta, iter_count);
                %disp(sort(K_test));
            end
    ...
    %     mkdir(outpath11);
    outpath_res_norm = sprintf('/home/jagdeep/Desktop/NMF/Experiments/ICML16/residual_norm/beta_%d',beta);
